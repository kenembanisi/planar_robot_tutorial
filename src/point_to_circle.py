...
def draw_circle():
    ...
    # Set first reach point
    point = JointTrajectoryPoint()
    q, X, A = circle_IK(radius, duration, 0)
    ...
    traj.points[0].time_from_start = rospy.Duration.from_sec(0)

    for i in range(n-1):

        # Call the circle_IK function
        q, X, A = circle_IK(radius, duration, (i+1)*dt*duration)
        ...
        traj.points[(i+1)].time_from_start = rospy.Duration.from_sec((i+1)*dt*duration)
        rospy.loginfo("Status: joint_angles[%f][%f, %f]", (i+1)*dt*duration, q[0], q[1])
